...
*   **MCP服务器管理** (在群组中由全局管理员发送，会触发交互式配置流程)
    *   **说明**: 以下指令用于管理MCP (Model Context Protocol) 服务器。它们会启动一个多步骤的对话流程（状态机），机器人会依次询问配置所需的各项信息。请按照机器人的提示逐步回复。
        ...
        *   **`编辑MCP服务器` 指令的交互流程示例：**
            ...
            6.  机器人回复: `请输入新的超时时间（秒）：`
            7.  管理员发送: `120`
            8.  机器人回复: `✅ MCP服务器 'filesystem' 超时时间已更新为: 120s`
            **编辑环境变量示例（选择字段3，需要2步输入）：**
            5.  管理员发送: `3`
            6.  机器人回复: `请输入要设置的环境变量名（注意：这是写入操作）：`
            7.  管理员发送: `API_KEY`
            8.  机器人回复: `请输入环境变量 API_KEY 的值：`
            9.  管理员发送: `my-secret-key`
            10. 机器人回复: `✅ MCP服务器 'filesystem' 环境变量 'API_KEY' 已设置。`
        *   **`删除MCP服务器` 指令的交互流程示例：**
            1.  管理员发送: `删除MCP服务器`
...
3.  **图片OCR识别 (需启用 `EnableAutoOCR`)**
    ...
    *   **长图处理**: 对高度超过 4096 像素且高宽比不低于 2.5 的网页截图、聊天记录等纵向长图，PaddleOCR 会自动在空白横带附近切片并按顺序合并结果；找不到安全空白时会使用带重叠的切片，避免在文字中间直接截断。普通图片仍按原流程单次识别。
