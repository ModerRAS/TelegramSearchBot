...
namespace TelegramSearchBot.Service.Manage {
    public class EditLLMConfService : IService {
        ...
        public async Task<(bool, string)> ExecuteAsync(string Command, long ChatId) {
            ...
            // 处理初始命令
            var initialResult = await HandleInitialCommand(Command, db, stateKey, dataKey);
            if (initialResult != null) return initialResult.Value;
        ...
        private async Task<(bool, string)?> HandleInitialCommand(string command, IDatabase db, string stateKey, string dataKey) {
            if (command.Trim().Equals("刷新所有渠道", StringComparison.OrdinalIgnoreCase)) {
            ...
            else if (command.Trim().Equals("新建渠道", StringComparison.OrdinalIgnoreCase)) {
                ...
                return await HandleChannelEditing(command, db, stateKey, dataKey, initial: true);
            }
                ...
                return (true, "请选择要移除模型的渠道ID：");
            }
            else if (command.Trim().Equals("查看模型", StringComparison.OrdinalIgnoreCase)) {
                return await HandleModelManagement(command, db, stateKey, dataKey, "viewing_model_select_channel");
            }
            return null;
        }

        private async Task<(bool, string)> HandleChannelCreation(string command, IDatabase db, string stateKey, string dataKey, string currentState) {
            switch (currentState) {
                case "awaiting_name":
                    await db.StringSetAsync(dataKey, command); // 存储名称
                    await db.StringSetAsync(stateKey, "awaiting_gateway");
        ...
        private async Task<(bool, string)> HandleChannelEditing(string command, IDatabase db, string stateKey, string dataKey, bool initial = false) {
            if (initial) {
        ...
        private async Task<(bool, string)> HandleModelManagement(string command, IDatabase db, string stateKey, string dataKey, string targetState) {
            if (targetState == "viewing_model_select_channel") {
                var channels = await GetAllChannels();
                if (channels.Count == 0) {
                ...
                await db.StringSetAsync(stateKey, targetState);
                return (true, sb.ToString());
            }
        ...
        private async Task<(bool, string)> HandleConfiguration(string command, IDatabase db, string stateKey, string dataKey, string currentState) {
            switch (currentState) {
                case "setting_alt_photo_model":
                    try {
                        var config = await DataContext.AppConfigurationItems
